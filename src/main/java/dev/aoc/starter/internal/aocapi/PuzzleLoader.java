...
public record PuzzleLoader(String token) {
        ...
        private static final Map<String, String> HEADERS = Map.of(
                        com.google.common.net.HttpHeaders.ACCEPT, MediaType.PLAIN_TEXT_UTF_8.toString());

        @SneakyThrows
        public String load(PuzzleDetails puzzleDetails) {

                var uriString = String.format("https://adventofcode.com/%s/day/%s/input", puzzleDetails.year(),
                                puzzleDetails.day());
                ...
                Preconditions.checkState(
                                ...
                                String.format("Unexpected status code from %s: %d: %s: %s", uriString,
                                                response.statusCode(), response.body(), token));
